...
int TmemPut(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    ret = tmem_ioctl_put(key, key_len, value, value_len, 0,NULL);
...
/*custom command*/
int TmemPutTime(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    key = RedisModule_StringPtrLen(argv[1], &key_len);
    value = RedisModule_StringPtrLen(argv[2], &value_len);
    ret = tmem_ioctl_put(key, key_len, value, value_len, 0,&times);
